...
- **S3** — source of truth. Each ingested item is written as an atomic `.md` file with YAML frontmatter (source, date, confidence scores, relationship metadata). Human-readable, portable, enables Obsidian sync via `aws s3 sync`. A separate `slip-box-uploads` bucket (`DESTROY` + 14-day lifecycle expiry) stages raw uploaded PDFs before `read_pdf` reads them — deliberately not `slip-box-notes`, since that bucket is the KB's entire unscoped data source and a raw PDF landing there would get auto-embedded directly, bypassing classification.
...
- **DynamoDB** — `items` table (structured metadata for all note types: `literature-note`, `permanent-note`; `recent-index` GSI — constant `gsi_pk`, sort key `created_at` — so the Recent list can `Query` newest-first instead of an unordered `Scan`; `source-index` GSI, keyed on `source_id`, answers "every note from this source"), `edges` table (`from_id`, `to_id`, `type`, `confidence`, `history`; `to_id-index` GSI for reverse lookups), and `sources` table (`source_id`, `source_key`, `title`, `author`, `type: web|youtube|pdf`, `url`, `retrieved_at`; `source-key-index` GSI on `source_key` — a normalized URL for web/YouTube, or a content hash (the S3 object's ETag) for PDF — for write-time dedup, so the same source cited by multiple notes resolves to one shared record). Notes reference their source via `source: [[source-id|Title]]` in frontmatter, same wikilink pattern as edges. Neptune is the production graph target but DynamoDB covers all MVP query needs without VPC complexity.
  - `items` also carries two sparse attributes, both absent (not `null`/`[]`) rather than stored empty, so `attribute_not_exists()`/`attribute_exists()` filtering works directly with no new GSI: `reviewed_at` (ISO timestamp, set only via an explicit review action, cleared automatically on a body-changing hand-edit) and `index_keywords: list[str]` (which curated keyword(s), if any, this note is an entry point for — Luhmann's second, deliberately sparse index alongside the numbered slips, distinct from `tags` by being hand-picked rather than exhaustive).
...
- **Expo / React Native** (`app/expo/`) — the primary app for using the slip box day to day, not just capturing into it. Tab navigation (Slip Box / Add Source / Review): the Slip Box tab is a box switcher (Index / Recent / Summaries — Luhmann's own physically-separate-boxes idea, a switcher instead of a type label on every card) over a flip-through note detail view (`note/[noteId].tsx`) that pages through a note's own connections one at a time, central note "out," never landing on a peripheral note first. Review groups the unreviewed queue by source into swipeable stacks (`review-stack.tsx`), each note actionable inline (tag via Index Cards, delete, mark reviewed) without leaving the stack. Add Source is share-sheet capture (`expo-share-intent`, one config plugin for both iOS and Android) plus in-app paste-a-link-or-text — both share one `useIngestFlow` hook. Multi-select any two-plus notes in the Slip Box tab to synthesize an on-demand summary card (`POST /summarize`), reusing the exact same async-invoke-and-poll plumbing as ingestion. Same backend as the web app; API key stored on-device via `expo-secure-store` rather than a server-side proxy, since there's no server tier in a mobile app to hold it. See `app/expo/README.md`.
