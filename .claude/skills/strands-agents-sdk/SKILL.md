...
- **`file_read`** — multi-format file reading. Its `document` mode does **not** extract PDF text in Python — it reads the raw bytes and returns a Bedrock **document content block** (`ToolResultContent`'s native `document` key; confirmed in its source and in `strands/types/tools.py`), so the model reads the PDF natively (text, tables, layout) on its next turn. This project's own `read_pdf` tool (`app/MyAgent/tools/notes.py`) uses the exact same technique, hand-written rather than adding this whole package for one capability — see `docs/build-log.md`'s PDF ingestion entry.
