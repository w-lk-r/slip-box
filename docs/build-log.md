...
- Not yet deployed — `npm run deploy:api` is next, left for Jonathan to run per the established workflow (same as `agentcore deploy`).

## Up Next

- [ ] Deploy (`cd agentcore/cdk && npm run deploy:api`) and verify end-to-end against the real stack: `POST /ingest` → poll `/items` → `/graph` → edge edit → edge delete, plus a deliberate 403 (missing/wrong API key) to confirm the guardrail is live. Once passed, mark review-todo #11 RESOLVED.
